"""
祥哥籌碼價量戰情室 v3.5
1. 成交量優化：改為「張數」單位，且能量柱顯示紅綠色彩。
2. 手機順滑：持續禁用圖表縮放拖曳，解決手機捲動問題。
3. 功能完整：保留 15 級定義、散戶線、人均張數及所有診斷邏輯。
"""
...
with st.sidebar:
    ...
    selected_stock = st.selectbox("搜尋標的", options=stock_options, index=stock_options.index("2330 台積電") if "2330 台積電" in stock_options else 0)
    sid = selected_stock.split(" ")[0]
    sname = stock_map.get(sid, "")
    d_range = st.date_input("選擇區間", [datetime.now() - timedelta(days=120), datetime.now()])
    price_freq = st.radio("價量頻率", ["日資料", "週資料 (同步籌碼)"], index=0)
...
with tab2:
    ...
    for d, sub in df_chip.groupby("資料日期"):
        ...
        weekly_rows.append({"日期": d, "股價": round(p_close, 2), "成交張數": int(p_vol), "大戶%": round(bw, 2), "中間戶%": round(mw, 2), "散戶%": round(sw, 2), "總人數": int(tp), "人均張數": round((ts/tp)/1000, 2) if tp > 0 else 0, "集中度": round(bw + mw, 2)})
    ...
    # 1. 總股東人數：柱子加粗
    fig.add_trace(go.Bar(x=res["日期"], y=res["總人數"], name="人數", marker_color="royalblue", opacity=0.8, width=432000000), row=3, col=1)
    ...
    if not df_price.empty:
        ...
        # 成交量彩色邏輯：當期收盤 >= 開盤顯紅，否則綠[cite: 12]
        vol_colors = ['red' if c >= o else 'green' for o, c in zip(p_plot['Open'], p_plot['Close'])]
        fig.add_trace(go.Bar(x=p_plot.index, y=p_plot["Volume"] / 1000, name="張數", marker_color=vol_colors, opacity=0.8), row=2, col=1)
    ...
    st.dataframe(view.style.format({"大戶%": "{:.2f}", "中間戶%": "{:.2f}", "散戶%": "{:.2f}", "大戶增減": "{:+.2f}", "總人數": "{:,.0f}", "人均張數": "{:.2f}"}).map(lambda x: "color: red" if isinstance(x, (int, float)) and x > 0 else "color: green" if isinstance(x, (int, float)) and x < 0 else "", subset=["大戶增減", "人數增減"]), use_container_width=True)

    # ── 祥哥量化報告文字：100% 還原[cite: 12] ──
    st.divider()
    st.subheader(f"📊 {sid} 祥哥報告")
    if len(res) >= 2:
