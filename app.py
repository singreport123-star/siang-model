"""
祥哥籌碼價量戰情室 v3.2
功能完整保留 v3.1：包含 diff 方向、診斷文字、MultiIndex 處理。
優化：1. 總人數柱狀圖加粗 2. 詳細指標小數點固定 3. 手機響應式佈局。
"""
...
with tab1:
    ...
    if os.path.exists("latest_snapshot.parquet"):
        ...
        def _color_num(val):
            return "color: red" if isinstance(val, (int, float)) and val > 0 else "color: green" if isinstance(val, (int, float)) and val < 0 else ""
        display_cols = ["股號", "名稱", "大戶%", "大戶週增減", "人數變動", "集中度(大+中)"]
        st.dataframe(df_rank[display_cols].style.map(_color_num, subset=["大戶週增減", "人數變動"]).format("{:.2f}", subset=["大戶%", "大戶週增減", "人數變動", "集中度(大+中)"]), use_container_width=True, height=600)
    else:
        st.info("尚未偵測到全市場快照資料，請先執行 scanner.py。")

# ═══════════════════════════════════════════════════════════════
# Tab 2：個股深度分析[cite: 12]
# ═══════════════════════════════════════════════════════════════
with tab2:
    st.header(f"📈 {sid} {sname} 深度戰情看板")
    path = Path("data/chip") / sid[:2] / f"{sid}.parquet"
    if not path.exists() or len(d_range) != 2:
        st.warning("請選擇完整日期或等待資料載入...")
        st.stop()
    ...
    def _agg_levels(sub, levels):
        mask = sub["持股分級"].isin(levels)
        return float(sub.loc[mask, "權重"].sum()), float(sub.loc[mask, "人數"].sum()), float(sub.loc[mask, "股數"].sum())
    ...
    for d, sub in df_chip.groupby("資料日期"):
        ...
        bw, bp, bs = _agg_levels(sub, big_lv); mw, mp, ms = _agg_levels(sub, mid_lv); sw, sp, ss = _agg_levels(sub, small_lv)
    ...
    res["診斷"] = res.apply(lambda row: "🔴 強力吸籌" if row["大戶增減"] > 0 and row["人數增減"] < 0 else ("🟡 主力加碼" if row["大戶增減"] > 0 else ("🟠 主力減碼" if row["大戶增減"] < 0 else "⚪ 中性觀望")), axis=1)
    ...
    # 格式優化：柱狀圖寬度設為 5 天 (5*24*60*60*1000 毫秒)
    fig.add_trace(go.Bar(x=res["日期"], y=res["總人數"], name="總人數", marker_color="royalblue", opacity=0.8, width=432000000), row=3, col=1)

    fig.update_layout(height=750, margin=dict(l=10, r=10, t=30, b=10), hovermode="x unified", xaxis_rangeslider_visible=False, template="plotly_dark", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
