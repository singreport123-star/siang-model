"""
祥哥籌碼價量戰情室 v4.6 - 終極定稿版
1. 修正單位：僅將排行榜「人數變動」標籤改為 (千人)，不更動換算邏輯。
2. 功能定案：保留 v4.4 所有診斷定義、彩色張數、手機優化及穩定解析邏輯。
"""
...
with st.sidebar:
    ...
    st.caption("Powered by 祥哥籌碼模型 v4.6")

st.title("🚀 祥哥籌碼價量戰情室 (完全體)")
tab1, tab2 = st.tabs(["📊 市場排行榜", "🔍 個股深度分析"])

# ═══════════════════════════════════════════════════════════════
# Tab 1：全市場排行榜 (僅修正單位名稱)
# ═══════════════════════════════════════════════════════════════
with tab1:
...
with tab2:
    ...
    with st.expander("ℹ️ 祥哥模型指標定義說明"):
        ...
        * **大戶/股價相關性**: 計算大戶持股比例與收盤價的相關係數。
        ...
        ### ⚖️ 綜合判斷標準
        * **🔴 強力吸籌**: 大戶持股比例增加，且總人數顯著下降。
        * **🟡 主力加碼**: 大戶持股比例單週呈增加趨勢。
        * **🟠 主力減碼**: 大戶持股比例單週呈減少趨勢。
        * **⚪ 中性觀望**: 籌碼變動不明顯。
        * **⚠️ 籌碼渙散**: 大戶持股比例大幅下降且總人數增加。
        """)
